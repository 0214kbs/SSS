...
const MakesruveyComponent = (props: any) => {
  ...
  const { title, setTitle, content, closedHeadCount, startTime, endTime, type, surveyTarget, img } = useSettingSurveyApiStore();
  ...
  const handleCreateButtonClick = () => {
    ...
    const Outer_hasEmptyValue =
      surveyData.title === ''
        ? (() => {
            setModalText('설문 제목을 입력해주세요');
            return true;
          })()
        : surveyData.content === ''
          ? (() => {
              setModalText('설문 부가 설명을 입력해주세요.');
              return true;
            })()
          : surveyData.closedHeadCount === 0
            ? (() => {
                setModalText('설문 인원을 입력해주세요.');
                return true;
              })()
            : surveyData.endTime === 0
              ? (() => {
                  setModalText('종료 시간을 선택해주세요.');
                  return true;
                })()
              : surveyData.startTime === 0
                ? (() => {
                    setModalText('시작 시간을 선택해주세요.');
                    return true;
                  })()
                : surveyData.surveyTarget === undefined || surveyData.surveyTarget.length === 0
                  ? (() => {
                      setModalText('설문 대상을 선택해주세요.');
                      return true;
                    })()
                  : surveyData.type === ''
                    ? (() => {
                        setModalText('설문 유형을 선택해주세요.');
                        return true;
                      })()
                    : surveyData.startTime === ''
                      ? (() => {
                          setModalText('설문 시작 시간을 선택해주세요.');
                          return true;
                        })()
                      : surveyData.endTime === ''
                        ? (() => {
                            setModalText('설문 마감 시간을 선택해주세요.');
                            return true;
                          })()
                        : surveyComponents.length === 0
                          ? (() => {
                              setModalText('설문 문항은 최소 1개 이상 존재해야 합니다.');
                              return true;
                            })()
                          : false;
